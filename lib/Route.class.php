...
class Route {
    ...
    private function __construct() {
        ...
        if(!is_array($this->routes))
            Log::record('Votre fichier de routes est mal formaté', Log::WARNING);
    }
    ...
    public function run() {
        if(isset($_SERVER['REQUEST_URI'])) {
            ...
            if(preg_match('#(.+)/$#i', $url)) $url = substr($url, 0, -1);
            ...
            if(!$find && $url!='/') {
                ...
                if(isset($_SERVER['HTTP_REFERER'])) {
                    $txt = ' (page précédente : '.$_SERVER['HTTP_REFERER'].')';
                }
                Log::record('Erreur 404 - la page '.$url.' n\'existe pas'.$txt, Log::ERROR);
                Error::error(404, array('error'=>array('La page '.$url.'  n\'existe pas')));
            }
        }
    }
    ...
    public function searchRoute($url) {
        ...
        foreach($this->routes as $route) {
            ...
            if(isset($url['params']) && !isset($route['params'])) $ok = false;
            elseif(isset($route['params']) && !isset($url['params'])) $ok = false;
    ...
    private function check($url, $route) {
        ...
        if(count($route_explode)!=count($url_explode)) $idem=false; 
        
        while( $idem && $i<count($route_explode) ) {
            // différents si paramètres différents
            if($route_explode[$i]!=$url_explode[$i]) $idem = false;
