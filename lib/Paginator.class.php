...
class Paginator {
	...
	public function pageByItem($item) {
		...
		do {
			if($item == $this->items[$i]) $page = ceil(($i+1)/$this->nbItemsPerPage);
	...
	public function positionByItem($item) {
		...
		do {
			if($item == $this->items[$i]) $find=true;
			else $i++;
	...
	public function itemByPosition($pos) {
		if($pos<0) $pos=count($this->items)-1;
		if($pos>=count($this->items)) $pos=0;
	...
	public function items($all=false) {
		...
		if($nbItems<=$this->nbItemsPerPage || $all) { $array = $this->items; }
		else {
	...
	public function nbItemsPerPage() { return $this->nbItemsPerPage; }
	public function currentPage() { return $this->currentPage; }
	public function nbPage() { return $this->nbPage; }
	...
	public function _nbItemsPerPage($nbItemsPerPage) {
		if($nbItemsPerPage>count($this->items)) $nbItemsPerPage=count($this->items);
		if($nbItemsPerPage<0) $nbItemsPerPage=0;
	...
	public function _currentPage($currentPage) {
		...
		if($currentPage>$this->nbPage) $currentPage=$this->nbPage;

		$this->currentPage = $currentPage;
	}
	private function _nbPage() {
		if($this->nbItemsPerPage>0) $this->nbPage = ceil(count($this->items)/$this->nbItemsPerPage);
	}
}
