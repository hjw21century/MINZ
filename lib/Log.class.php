...
class Log {
    ...
    public static function record($information, $level, $file=null) {
        if(Configuration::environment()!=Configuration::SILENT
           && !($level==Log::WARNING && Configuration::environment()==Configuration::PRODUCTION)
           && !($level==Log::NOTICE && Configuration::environment()==Configuration::PRODUCTION)):
            
            if( is_null($file) ) $file = LOG_PATH.'/application.log';
