...
class View {
	...
	/**
     * $fic_view chemin du fichier de vue
     */
	private $fic_view;
	...
	public function __construct() {
        ...
        $this->fic_view = APP_PATH.'/views/'.$route->controller().'/'.$route->action().'.phtml';

        if(Configuration::use_layout()) $this->layout = true;
    }
	...
	public function render() {
        if(file_exists($this->fic_view)) include($this->fic_view);
        else Log::record('Aucune vue n\'est associée au contrôleur : '.$this->fic_view, Log::NOTICE);
	}
    ...
    public function renderWithLayout() {
        $fic_layout = APP_PATH.'/layout/layout.phtml';
        if(file_exists($fic_layout)) include($fic_layout);
        else {
            $this->render();
            Log::record('Le fichier de layout n\'existe pas : '.$fic_layout, Log::NOTICE);
        }
    }
	...
	public function partial($file) {
        $fic_partial = APP_PATH.'/layout/'.$file.'.phtml';
        if(file_exists($fic_partial)) include($fic_partial);
        else Log::record('Le fichier '.$file.'.phtml n\'existe pas : '.$fic_partial, Log::WARNING);
	}
